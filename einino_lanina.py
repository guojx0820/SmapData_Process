...
def draw_fig(lon, lat, ssta):
    ...
    axes[0].set_title('SSTA of 2015/' + str(i % 12 + 1) + ' EI Nino')
    ...
    axes[1].set_title('SSTA of 2000/' + str(j % 12 + 1) + ' La Nina')
    map = Basemap(projection='cyl', resolution='l', llcrnrlat=-30., urcrnrlat=30., llcrnrlon=60., urcrnrlon=301.,
                  ax=axes[1],
                  lat_0=0,
                  lon_0=-180)
    ...
    plt.imshow(ssta[j])
    plt.savefig(outputfile + '2000_2015_' + str(i % 12 + 1) + '.jpg', bbox_inches='tight', dpi=300)
    plt.show()
...
if __name__ == '__main__':
    ...
    for i, j in zip(range(288, 300, 11), range(108, 120, 11)):
